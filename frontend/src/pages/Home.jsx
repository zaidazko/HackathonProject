...
const App = () => {
  ...
  const handleCheckboxChange = (value, list, setter, maxSelections = Infinity) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!budget || budget <= 0) return setError("Please enter a valid budget amount.");
  ...
  const openCamera = useCallback(async () => {
    ...
    try {
      ...
      setCameraError("Camera access denied or unavailable. Opening file picker…");
  ...
  const handleAddToGallery = useCallback(async () => {
    try {
      ...
      const file = new File([blob], "generated-design.png", { type: blob.type || "image/png" });
  ...
  const totalCostRange =
    result?.furniture.reduce(
      (range, item) => {
        if (item.searchResults?.length) {
          const prices = item.searchResults.map((p) => p.price);
          return { min: range.min + Math.min(...prices), max: range.max + Math.max(...prices) };
        }
      ...
      <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        <div className="max-w-4xl mx-auto grid gap-10">
          <div className="p-8 bg-white rounded-2xl shadow-lg border border-slate-200">
            <h2 className="text-3xl font-bold mb-8 text-slate-800 text-center">1. Upload Your Room</h2>

            <div className="p-8 border-2 border-dashed border-slate-300 rounded-xl text-center bg-slate-50" onPaste={handlePaste} tabIndex={0}>
              {imagePreview ? (
                ...
                <div className="flex flex-col items-center justify-center space-y-4">
                  <div className="space-y-4">
                    ...
                    <p className="text-slate-600 text-lg">
                      Upload a picture, paste from clipboard, or use your camera to capture it
                    </p>
                    <p className="text-sm text-slate-500 mt-2">💡 Tip: You can also paste an image directly (Ctrl+V / Cmd+V)</p>
                  </div>
                </div>
              )}
            </div>
          ...
          {/* Preferences */}
          <div className="p-8 bg-white rounded-2xl shadow-lg border border-slate-200">
            <h2 className="text-3xl font-bold mb-8 text-slate-800 text-center">2. Your Style Preferences</h2>
            ...
            <div className="space-y-10">
              ...
              <div className="bg-slate-50 rounded-xl p-6">
                <h3 className="text-xl font-semibold mb-4 text-slate-700 flex items-center">
                  ...
                  Design Features
                  <span className="text-sm font-normal text-slate-500 ml-2">(Select up to 2)</span>
                </h3>
                <p className="text-slate-600 mb-6">Choose the design styles that appeal to you most</p>
                  ...
                  {DESIGN_STYLES.map((style) => (
                    ...
                    >
                      <input
                        ...
                        disabled={!selectedStyles.includes(style) && selectedStyles.length >= 2}
                        onChange={() => handleCheckboxChange(style, selectedStyles, setSelectedStyles, 2)}
                      />
                      <span className={`font-medium ${selectedStyles.includes(style) ? "text-indigo-700" : "text-slate-700"}`}>{style}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-slate-50 rounded-xl p-6">
                ...
                <p className="text-slate-600 mb-6">Select color schemes that match your vision</p>
                    ...
                    >
                      <div className="flex items-center space-x-3 mb-3">
                        <input
                          ...
                          disabled={!selectedColors.includes(paletteName) && selectedColors.length >= 2}
                          onChange={() => handleCheckboxChange(paletteName, selectedColors, setSelectedColors, 2)}
                        />
                        <span className={`font-medium ${selectedColors.includes(paletteName) ? "text-indigo-700" : "text-slate-700"}`}>{paletteName}</span>
                      </div>
                      <div className="flex space-x-1">
                        {colors.map((color, index) => (
                          <div key={index} className="w-6 h-6 rounded-full border border-slate-200 shadow-sm" style={{ backgroundColor: color }} title={color} />
                        ))}
                      </div>
                    </label>
                  ))}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-slate-50 rounded-xl p-6">
                  ...
                  <p className="text-slate-600 mb-4">Set your furniture budget</p>
                ...
                <div className="bg-slate-50 rounded-xl p-6">
                  ...
                  <p className="text-slate-600 mb-4">Any specific requirements or preferences?</p>
                  <textarea
                    value={instructions}
            ...
            <div className="p-8 bg-white rounded-2xl shadow-lg border border-slate-200 animate-fade-in">
                ...
                {result.furniture.map((item, index) => (
                  <div key={index} className="border border-slate-200 rounded-lg p-6 bg-slate-50">
                    <div className="mb-4">
                      ...
                      <p className="text-sm font-medium text-slate-700">Estimated Price Range: ${item.estimatedPrice}</p>
                    </div>
                    {item.searchResults?.length ? (
                      <div>
                        <h5 className="text-md font-medium text-slate-700 mb-3">Similar Products Found:</h5>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                          {item.searchResults.slice(0, 4).map((product, i) => (
                            <div key={i} className="bg-white border border-slate-200 rounded-lg p-3 hover:shadow-md transition-shadow flex flex-col">
                              {product.thumbnail && (
                                <div className="relative mb-3">
                                  ...
                                  />
                                  <div className="w-full h-24 bg-slate-100 flex items-center justify-center text-slate-400 text-xs rounded-md" style={{ display: "none" }}>
                                    No Image
                                  </div>
                                </div>
                              )}
                              <div className="flex-grow">
                                <h6 className="font-medium text-slate-800 text-xs mb-2 line-clamp-2">{product.title}</h6>
                                <p className="text-sm font-bold text-indigo-600 mb-1">${product.price}</p>
                                <p className="text-xs text-slate-500 mb-2">{product.source}</p>
                              </div>
                              <div className="mt-auto">
                                <a href={product.link} target="_blank" rel="noopener noreferrer" className="inline-block w-full text-center bg-indigo-600 text-white text-xs font-medium py-2 px-3 rounded-md hover:bg-indigo-700 transition-colors">
                                  View Product
                                </a>
                              </div>
                            </div>
              ...
              <div className="mt-8 p-6 bg-slate-100 rounded-lg">
                <div className="flex justify-between items-center">
                  <span className="text-lg font-semibold text-slate-800">Estimated Total Cost Range:</span>
                  <div className="text-right">
                    <span className={`text-2xl font-bold ${totalCost > budget ? "text-red-600" : "text-green-600"}`}>
                      ${totalCostRange.min.toFixed(0)} - ${totalCostRange.max.toFixed(0)}
                    </span>
                    <p className="text-sm text-slate-600 mt-1">Average: ${totalCost.toFixed(0)}</p>
                  </div>
                </div>
                {totalCost > budget && (
                  <p className="text-sm text-red-600 mt-2 text-right">Note: Estimated range is over your specified budget of ${budget}.</p>
                )}
                <p className="text-xs text-slate-500 mt-2 text-center">Price range based on available product options for each item</p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mt-8">
                ...
                <button
                  ...
                  className={`inline-flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-full text-white transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                    addingToGallery ? "bg-indigo-400 cursor-not-allowed" : "bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500"
                  }`}
                >
                  {addingToGallery ? "Adding…" : "Add to Gallery"}
                </button>
              </div>
              {addToGalleryMsg && <p className="text-center mt-3 text-sm">{addToGalleryMsg}</p>}
            </div>
          )}
        </div>
      </main>
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-4 w-full max-w-xl shadow-xl">
            ...
            <div className="flex justify-end gap-3 mt-4">
              <button onClick={closeCamera} className="px-4 py-2 rounded-lg bg-gray-200 hover:bg-gray-300">Cancel</button>
              <button onClick={capturePhoto} className="px-4 py-2 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700">Capture</button>
            </div>
