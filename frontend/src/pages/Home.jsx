...
const CameraCapture = ({ isOpen, onClose, onCapture }) => {
  ...
  const handleCapture = useCallback(async () => {
    ...
    // Use video’s intrinsic resolution for best quality
  ...
  // Start/stop stream when modal toggles
  React.useEffect(() => {
    ...
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="p-4 border-b flex items-center justify-between">
          <h3 className="text-lg font-semibold text-slate-800">Take a Photo</h3>
          <button
            onClick={() => { onClose(); }}
        ...
        <div className="p-4 flex gap-3 justify-end">
          <button
            onClick={() => { onClose(); }}
            className="px-4 py-2 rounded-full border border-slate-300 text-slate-700 bg-white hover:bg-slate-50"
          >
            Cancel
          </button>
          <button
            onClick={handleCapture}
            className="px-4 py-2 rounded-full text-white bg-indigo-600 hover:bg-indigo-700"
          >
            Capture
          </button>
        </div>
...
const App = () => {
  ...
  // NEW: camera modal control
  const [isCameraOpen, setIsCameraOpen] = useState(false);

  const fileInputRef = useRef(null);
  const cameraInputRef = useRef(null); // fallback <input capture="environment">
  ...
  // NEW: open camera with getUserMedia; fallback to native file input
  const openCamera = async () => {
    if (navigator.mediaDevices?.getUserMedia) {
      try {
        // Probe permission quickly to avoid opening modal then failing
        const stream = await navigator.mediaDevices.getUserMedia({ video: true });
        // stop the probe stream immediately; actual stream starts in CameraCapture
    ...
    // Fallback: trigger native picker (mobile honors capture="environment")
    cameraInputRef.current?.click();
  };

  // NEW: receive captured file from CameraCapture and reuse existing flow
  ...
  const totalCost =
    result?.furniture.reduce((sum, item) => sum + item.price, 0) || 0;
  ...
  return (
    <div className="min-h-screen font-sans text-slate-800">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {DESIGN_STYLES.map(style => (
                    <label key={style} className="flex items-center space-x-2 cursor-pointer p-2 rounded-md hover:bg-slate-100 transition-colors">
                      <input
                        type="checkbox"
                        className="h-5 w-5 rounded border-gray-300 text-teal-600 focus:ring-teal-500 custom-checkbox"
                        checked={selectedStyles.includes(style)}
                        onChange={() => handleCheckboxChange(style, selectedStyles, setSelectedStyles)}
                      />
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-7 pr-4 py-2 border border-slate-500 bg-slate-600 text-white rounded-md focus:ring-indigo-500 focus:border-indigo-500 placeholder-slate-300"
              ...
              <div className="md:col-span-2">
                ...
                <textarea
                  ...
                  className="w-full h-24 p-2 border border-slate-500 bg-slate-600 text-white rounded-md focus:ring-indigo-500 focus:border-indigo-500 placeholder-slate-300"
                  placeholder="e.g., Add a large plant in the corner, make sure there's a cozy reading nook..."
                />
              </div>
            </div>
          </div>
              ...
              <div className="overflow-x-auto">
                ...
                {totalCost > budget && (
                  <p className="text-sm text-red-600 text-right mt-2">
                    Note: Estimated total is over your specified budget of ${budget}.
                  </p>
                )}
              </div>
      ...
      <footer className="text-center py-6 text-sm text-slate-500"></footer>
